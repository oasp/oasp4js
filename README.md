...
Edit the `<tomcat_dir>\conf\server.xml` file. Change the Tomcat port to 8081 (if you don't want to change the Tomcat's port, then see the [hint below about how to configure a different port on the client side](#howToChangeTomcatsPortInConfigJson) ): 
